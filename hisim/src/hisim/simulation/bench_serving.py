...
# Adapted from https://github.com/sgl-project/sglang/blob/v0.5.6.post2/python/sglang/bench_serving.py
...
class BenchmarkMetrics:
    bench_mode: str
    completed: int
    duration: float  # The duration used to calculate throughput.
    total_input: int
...
def sample_hisim_collection_requests(
    ...
    if not dataset_path or not os.path.exists(dataset_path):
        raise ValueError(
            f"The dataset path[{dataset_path}] is not existed, which is needed by {args.dataset_name}."
        )
    ...
    if use_trace_timestamps:
        ...
        for request in input_requests_iter:
            ...
            if args.bench_mode == "simulation":
                # There is no need to sleep during simulation benchmark.
                pass
            else:
...
async def benchmark(
    ...
    # In simulation mode, elapsed time refers to the time spent running the benchmark.
    print("{:<40} {:<10.2f}".format("Elapsed time (s):", benchmark_duration))
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        default="simulation",
        help="Benchmark mode (simulation on/off)",
    )
    parser.add_argument(
        "--backend",
        type=str,
